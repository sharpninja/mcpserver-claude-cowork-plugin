...
_repl_workflow_requirements_is_mutation() {
    case "${1:-}" in
        createFr|updateFr|deleteFr|createTr|updateTr|deleteTr|createTest|updateTest|deleteTest|createMapping|deleteMapping|generateDocument|ingestDocument|copyAcceptanceCriteriaFromTodo) return 0 ;;
        *) return 1 ;;
    esac
}
...
# FR-MCP-REQACPLUGIN-001: probe workflow and typed get methods to retrieve the current
# stored entry for an updateX operation so partial updates preserve acceptanceCriteria.
_repl_requirements_existing_for_update() {
    ...
    get_method="$(_repl_requirements_update_get_method "$operation")"
    if [ -z "$get_method" ]; then
        return 1
    fi

    get_params="$(printf 'id: %s\n' "$id")"
    workflow_get_method="$(_repl_requirements_update_workflow_get_method "$operation" 2>/dev/null)"
...
_repl_requirements_typed_params() {
    local operation="$1"
    local params_yaml="${2:-}"
    local id title body fr_id doc_type format content documents_block source_format preferred_wiki_format existing
    ...
    case "$operation" in
        ...
        updateFr)
            ...
            title="$(_repl_yaml_get "$params_yaml" "title")"
            body="$(_repl_first_param_text "$params_yaml" "description" "body")"
            existing="$(_repl_requirements_existing_for_update "$operation" "$id" 2>/dev/null)"
            printf 'id: %s\nrequest:\n' "$id"
            _repl_yaml_field "  " "title" "$title"
            _repl_yaml_field "  " "body" "$body"
