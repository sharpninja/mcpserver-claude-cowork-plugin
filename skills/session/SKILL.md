...
To manage agent session logs, use the `workflow.sessionlog.*` REPL command namespace via `PowerShell.MCP wrapper`. Session logging captures agent activity, reasoning dialog, file operations, and design decisions as a structured audit trail.
...
- Agent name must be PascalCase (e.g. `ClaudeCowork`, `Copilot`, `Cursor`)
- Timestamp must be ISO 8601 compact UTC: `yyyyMMddTHHmmssZ`
- Suffix must be lowercase kebab-case: `feature-auth`, `bugfix-timeout`

Valid examples: `ClaudeCowork-20260409T120000Z-implement-auth`, `Copilot-20260304T113901Z-refactor-session`
...
payload:
  ...
  params:
    agent: ClaudeCowork
    sessionId: ClaudeCowork-20260409T120001Z-implement-auth
